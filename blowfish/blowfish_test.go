...
func TestCipherEncrypt(t *testing.T) {
	for i, tt := range encryptTests {
		c, err := blowfish.NewCipher(tt.key)
		if err != nil {
...
func TestSaltedCipherKeyLength(t *testing.T) {
	if _, err := blowfish.NewSaltedCipher(nil, []byte{'a'}); err == nil || err.Error() != blowfish.ErrKeySize {
		t.Errorf("NewSaltedCipher with short key, gave error %#v, expected %s", err, blowfish.ErrKeySize)
	}

	key := []byte("012345678901234567890123456789012345678901234567890123456")
	if _, err := blowfish.NewSaltedCipher(key, []byte{'a'}); err != nil {
...
func TestSaltedCipher(t *testing.T) {
	...
	for i, v := range saltedVectors {
		c, err := blowfish.NewSaltedCipher(key[:], salt[:i])
		if err != nil {
			t.Fatal(err)
		}
		var buf [8]byte
